# Attaché à un Area3D
# L'Area3D sert ici à détecter quand le joueur entre dans la zone de fin du labyrinthe
extends Area3D
...
# Vérifie que c'est bien le joueur qui sort
	if body.name == "Player":
		joueur_proche = false # joeur plus dans la zone
...
# Vérifie 2 conditions : si on est dans l'Area3D et qu'on appuie sur E (interact)
	if joueur_proche and Input.is_action_just_pressed("interact"):
		...
		# Récupère la scène actuellement chargée (= main)
		var gm = get_tree().current_scene

# Indique au GameManager que le labyrinthe est terminé
		gm.labyrinthe_fini = true
